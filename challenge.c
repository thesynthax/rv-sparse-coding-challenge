...
void sparse_multiply(
    ...
    row_ptrs[0] = 0; // first entry of row_ptrs is always 0

    // CSR population and multiplication in one pass
    for (int i = 0; i < rows; ++i) {
        y[i] = 0.0;
        const double *row = &A[i * cols]; // avoids extra memory loads and pointer arithmetic in inner loop

        for (int j = 0; j < cols; ++j) {
            double val = row[j]; // direct access to row elements
